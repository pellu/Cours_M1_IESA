...
if($q != ""){
	$q = strtolower($q);
	$longueur=iconv_strlen($q);
	for($i = 0; $i < count($listePays); $i++){
		if(stristr($q,substr($listePays[$i][3],0,$longueur))){
			$suggestion .= "<li>".$listePays[$i][3]."</li>";
		}
	}
}else{
	echo "Vous n'avez pas réalisé une recherche";
}
	echo $suggestion === "" ? "aucun résultat" : $suggestion;